...
BEGIN
    ...
    WHERE nro_restaurante = @rest_uuid AND nom_sucursal = 'Perukai - Nueva Córdoba';
...
DECLARE @cod_zona_salon VARCHAR(36), @cod_zona_barra VARCHAR(36);
SELECT @cod_zona_salon = cod_zona FROM zonas WHERE nom_zona='Salón Principal';
SELECT @cod_zona_barra = cod_zona FROM zonas WHERE nom_zona='Barra';
...
BEGIN
    ...
    WHERE nro_restaurante = @rest_uuid AND nom_sucursal = 'Perukai - Güemes';
...
BEGIN
    INSERT INTO zonas_sucursales (nro_restaurante, nro_sucursal, cod_zona, cant_comensales, permite_menores, habilitada)
    VALUES (@rest_uuid, @suc_2_uuid, @cod_zona_salon, 50, 1, 1);
